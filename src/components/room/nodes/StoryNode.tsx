    ...
    const nodeClasses = useMemo(
      () =>
        cn(
          ...
          isActive
            ? "bg-amber-50 dark:bg-status-warning-bg border-amber-400 dark:border-amber-600"
            : "bg-gray-50 dark:bg-surface-1 border-gray-300 dark:border-border"
        ),
      [isActive]
    );
          ...
          <div className="mt-3 space-y-2">
            ...
            {isActive ? (
              <div className="flex items-center gap-2" aria-live="polite">
                <div
                  className="flex-1 bg-amber-200 dark:bg-status-warning-bg rounded-full h-2"
                  role="progressbar"
                  aria-label="Voting in progress"
                  aria-valuemin={0}
                  aria-valuemax={100}
                >
                  <div className="bg-amber-500 dark:bg-status-warning-fg h-2 rounded-full w-0 animate-pulse" />
                </div>
                <span className="text-xs text-amber-700 dark:text-status-warning-fg font-medium">
                  Voting...
                </span>
              </div>
            ) : (
              <div className="flex items-center gap-2" aria-live="polite">
                <svg
                  className="w-4 h-4 text-green-600 dark:text-status-success-fg"
                  fill="none"
                  strokeLinecap="round"
                ...
                <span className="text-xs text-green-700 dark:text-status-success-fg font-medium">
                  Voting Complete
                </span>
              </div>
            )}
              ...
              {isGameOver && (
                <button
                  ...
                  className={cn(
                    ...
                    resetCooldown > 0
                      ? "bg-gray-100 dark:bg-surface-3 text-gray-400 dark:text-gray-500 cursor-not-allowed"
                      : "bg-gray-100 hover:bg-gray-200 dark:bg-surface-3 dark:hover:bg-surface-3/80"
                  )}
                  aria-label={
                    resetCooldown > 0
